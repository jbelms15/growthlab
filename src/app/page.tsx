...
function isSectionStarted(data: any, id: string): boolean {
  switch (id) {
...
export default function Dashboard() {
  ...
  // Building mode
  const [sectionStatus, setSectionStatus] = useState<{ id: string; label: string; timeline: string; done: boolean }[]>([])
  ...
  useEffect(() => {
    async function load() {
      ...
      if (camp) {
        ...
        setSectionStatus(PHASES.map(p => ({ ...p, done: isSectionStarted(ws, p.id) })))
      }
              ...
              {sectionStatus.map(s => (
                <div key={s.id} className="flex items-center gap-2 text-sm">
                  {s.done ? (
                    ...
                    <span className="w-3.5 h-3.5 rounded-full border-2 border-gray-200 shrink-0 inline-block" />
                  )}
                  <span className={s.done ? 'text-gray-700' : 'text-gray-400'}>{s.label}</span>
                  <span className="text-xs text-gray-300">{s.timeline}</span>
                </div>
              ))}
            </div>
